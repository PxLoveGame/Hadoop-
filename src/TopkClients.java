...
//		System.out.println("ClientProfitMap : " + key + " ==> " + value);
...
//	private TreeMap<Integer, List<Text>> sortedWords = new TreeMap<>();
	...
	private void addCustomer(LongWritable value, Text customerId){
		...
		for (Long v : customers.keySet()){
			if (v < min_value) min_value = v;
		}

		if (customers.size() >= k && value.get() > min_value){ // s'inscrit dans le top
			customers.remove(min_value);	 // faire de la place pour la nouvelle valeur
//			System.out.println("Removing value " + min_value);
//			System.out.println(customers);
		}

		if (value.get() > min_value || customers.size() < k){ // insérer la valeur ?
//			System.out.println("Adding " + custId + " for profit " + value + " ( lowest was "+ min_value +" )");

			customers.put(value.get(), custId);
//			customers.get(value.get()).add(customerId);
//			min_value = Math.max(value.get(), min_value);
		}
...
//		Collection<Map.Entry<Long, String>> entrySet = customers.entrySet();
		...
		for (Map.Entry<Long, String> entry : entrySet){
			...
			context.write(new Text(customerId), new LongWritable(profit));
//			System.out.println("Cleaning : " + customerId + " ==> " + entry.getKey());
//			}
		}
	}
}
...
//		System.out.println("ProfitReduce " + key + " ==> " + sum);
